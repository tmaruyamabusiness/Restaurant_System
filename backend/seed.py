...
async def seed():
    ...
    async with async_session() as db:
        # --- Seats (skip if already exist) ---
        existing_seat = (await db.execute(select(Seat).limit(1))).scalar_one_or_none()
        ...
        # --- Menu Categories and Items (skip if already exist) ---
        existing_cat = (await db.execute(select(MenuCategory).limit(1))).scalar_one_or_none()
        if existing_cat is None:
            ...
            for item in menu_items:
                db.add(item)
            print("  + Created 5 categories with 20 menu items")
        else:
            print("  - Menu categories already exist, skipping")

        # --- Users (skip if already exist) ---
        existing_admin = (await db.execute(select(User).where(User.username == "admin"))).scalar_one_or_none()
        ...
        existing_staff = (await db.execute(select(User).where(User.username == "staff"))).scalar_one_or_none()
    ...
    print("\nSeed completed!")
    print("  Login: admin@restaurant.local / admin123 (OWNER)")
    print("  Login: staff@restaurant.local / staff123 (STAFF)")
